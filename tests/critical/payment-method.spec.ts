...
test.describe("Critical - Payment Method Flow", () => {
  ...
  test(`${critical} completes booking with payment method`, async ({
    page,
    loginAsScenario,
  }) => {
  ...
  test(`${critical} allows retroactive payment method assignment`, async ({
    page,
    loginAsScenario,
  }) => {
    const authData = await loginAsScenario("owner");
    ...
    await expect(
      page.getByText("Falta registrar el método de cobro"),
    ).toBeVisible();

    await page.getByRole("button", { name: "Transf." }).click();

    await expect(
      page.getByText("Método de pago registrado con éxito."),
    ).toBeVisible();
  ...
  test(`${critical} reflects payment method in analytics`, async ({
    page,
    loginAsScenario,
  }) => {
    const authData = await loginAsScenario("owner");
    ...
    await expect(
      page.getByRole("heading", { name: "Métodos de cobro" }),
    ).toBeVisible();
