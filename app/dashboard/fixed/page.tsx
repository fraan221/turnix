...
import {
  preloadRecurringBookings,
  getCurrentUserWithBarbershop,
...
export default async function FixedBookingsPage() {
  ...
  if (!barbershopId) {
    return (
      <div className="flex justify-center items-center p-6 h-full text-center">
...
async function FixedBookingsLoader({
  ...
  // Si el usuario es barbero, solo le mostramos sus propios servicios
  const filteredServices =
    role === "OWNER"
      ? services
      : services.filter((s: any) => s.barber?.id === userId);
